...
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ matched_address_load_test)
});
...
;// CONCATENATED MODULE: ./src/scenarios/application/matched-address.load.test.ts
/**
 * Matched Address Load Test with DP Token Authentication (BlazeMeter Compatible)
 * Tests property address matching endpoint under normal load
 *
 * Test type: Load
 * Purpose: Validate /property/matched-address endpoint performance with authentication
 *
 * BlazeMeter Compatibility:
 * - Uses per-VU token caching instead of setup() function
 * - Token is fetched once per VU and cached for 1 hour
 * - Works reliably in both local k6 and BlazeMeter cloud
 *
 * Setup:
 * 1. Set environment variables in .env file:
 *    - BASE_URL: API base URL
 *    - DP_TOKEN_URL: Token service URL (required for all environments)
 *
 * 2. Configure test users in src/fixtures/test-data.json:
 *    - Add multiple netBankId and searchAddress values
 *
 * 3. Configure load profile in taurus.yml (VUs, stages, thresholds)
 *
 * 4. Run: bzt taurus.yml
 *
 * Note: When using Taurus/BlazeMeter, VU and stage configuration
 * is defined in taurus.yml instead of here.
 */
...
// Token TTL: 1 hour (3600000 ms)
const TOKEN_TTL_MS = 3600000;
/**
 * Main test function - tests /property/matched-address endpoint with authentication
 * BlazeMeter Compatible: Uses per-VU token caching and user rotation
 */
/* harmony default export */ function matched_address_load_test() {
    const user = getTestUserForVU();
    // Log test start on first iteration
    if (__ITER === 0) {
        console.log('='.repeat(60));
        console.log(`[VU ${__VU}] Starting Matched Address Load Test`);
        console.log(`Base URL: ${__ENV.BASE_URL}`);
        console.log(`NetBankId: ${user.netBankId}`);
        console.log(`Search Address: ${user.searchAddress}`);
        console.log(`Token TTL: ${TOKEN_TTL_MS / 1000}s`);
        console.log('='.repeat(60));
    }
    // Get or fetch DP token with caching (per VU, per user)
    ...
    if (dpToken === null) {
        console.log(`[VU ${__VU}] No token available — skipping iteration (token fetch failed or in cooldown)`);
        (0,external_k6_namespaceObject.sleep)(1);
        return;
    }
    // Generate authentication headers
    const headers = getAuthHeaders({ dpToken });
    // GET /property/matched-address?address=...
    // URL encode the address for the query parameter
    const encodedAddress = encodeURIComponent(user.searchAddress);
    const response = get(`/property/matched-address?address=${encodedAddress}`, { headers });
    // Validate response
    ...
    if (!checks) {
        console.error(`[VU ${__VU}] Check failed - Status: ${response.status}, Duration: ${response.timings.duration}ms`);
    }
...
exports.default = matched_address_load_test;

/******/ })()
;
//# sourceMappingURL=matched-address.load.test.js.map