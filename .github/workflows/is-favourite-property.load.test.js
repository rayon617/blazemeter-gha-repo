...
__webpack_require__.d(__webpack_exports__, {
  "default": () => (/* binding */ is_favourite_property_load_test)
});
...
;// CONCATENATED MODULE: ./src/scenarios/application/is-favourite-property.load.test.ts
/**
 * Is Favourite Property Load Test with DP Token Authentication (BlazeMeter Compatible)
 * Tests property favourite status endpoint under normal load
 *
 * Test type: Load
 * Purpose: Validate /property/isfavourite endpoint performance with authentication
 *
 ...
 *    - Add multiple netBankId and gnafId values
 *
...
/**
 * Main test function - tests /property/isfavourite endpoint with authentication
 * BlazeMeter Compatible: Uses per-VU token caching and user rotation
 */
/* harmony default export */ function is_favourite_property_load_test() {
    const user = getTestUserForVU();
    // Log test start on first iteration
    if (__ITER === 0) {
        console.log('='.repeat(60));
        console.log(`[VU ${__VU}] Starting Is Favourite Property Load Test`);
        console.log(`Base URL: ${__ENV.BASE_URL}`);
        console.log(`NetBankId: ${user.netBankId}`);
        console.log(`GnafId: ${user.gnafId}`);
    ...
    // GET /property/isfavourite?gnafId=...
    const response = get(`/property/isfavourite?gnafId=${user.gnafId}`, { headers });
    // Validate response
...
exports.default = is_favourite_property_load_test;

/******/ })()
;
//# sourceMappingURL=is-favourite-property.load.test.js.map